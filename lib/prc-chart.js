...
export function renderPRC(svg, { cbtMinH, direction, seek, avoid, sleep }) {
  ...
  if (sleep) addBand(svg, sleep[0], sleep[1], PALETTE.sleep, 0.09);

  // The two active arms of the curve.
  addBand(svg, cbtMinH, cbtMinH + PRC_MODERATE_H, PALETTE.advance, 0.14);
  addBand(svg, cbtMinH - PRC_MODERATE_H, cbtMinH, PALETTE.delay, 0.14);
  ...
  const label = el('text', {
    x: Math.min(Math.max(cx, 24), W - 24), y: PAD_TOP - 8,
    fill: PALETTE.curve, 'font-size': 9, 'text-anchor': 'middle',
