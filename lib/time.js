 ...
 * caused Clairo to emit every event a day early for departures east of
...
/**
 * Parse a `datetime-local` value ("2026-05-16T06:45") into a civil date and a
 * decimal hour, with no timezone attached.
 *
 * Deliberately not `new Date(str)`: that resolves the string against whatever
 * timezone the user's machine happens to be in. These strings mean local time
 * *at an airport*, so the caller pairs the result with `zonedTimeToUtc` and the
 * right zone. Parsing them as machine-local put schedules on the wrong day for
 * anyone planning a trip from outside the destination's timezone.
 */
export function parseWallClock(str) {
  ...
  return { civil: { y, m: mo - 1, d }, hour: hh + mm / 60 };
}
