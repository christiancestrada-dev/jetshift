...
export function lightWindow(cbtMinH, direction) {
  ...
  if (direction === 'advance') {
    const [, idealEnd] = [cbtMinH, cbtMinH + PRC_MODERATE_H];
    const start = Math.max(cbtMinH, wake);
    return { start, end: Math.max(start, idealEnd), band: prcAt(start - cbtMinH).band };
  }

  const idealStart = cbtMinH - PRC_MODERATE_H;
  const end = Math.min(cbtMinH, bed);
  return { start: Math.min(idealStart, end), end, band: prcAt(end - cbtMinH - 0.01).band };
}
